...
git reset回滚代码仓库：git reset --hard
...
- 将所有改动文件添加到缓存区，如果你实在不确信哪些文件是改动过的，你只需要使用git add --all

```bash
git add -all
或者
# 这个命令跟git add --all一样，但是不会记录删除操作。
git add .
```

这个命令会将当前目录下包括子目录下所有改动的文件提交到暂存区，注意只包括改动的文件、删除的文件，不改动的不会放到缓存区。

如你在本地删除了min.c 这个命令会把删除信息也记录进去，然后在提交的时候把仓库里对应的min.c也删除掉，也就是说你在本地做的删除操作会被记录，提交仓库时会删除同样的文件，如果不想删除文件，可以使用git add .，注意后面有一个“.”点的符号。
...
可以看到min.c文件可回滚版本较少，在使用git reset命令将其回滚就可用了，命令格式如下：

```bash
git reset 1a1e91bf37add6c3914ebf20428efc0a7cea33f3 min.c
```

回退完成之后想要再次提交可以使用git add和git commit提交到本地仓库中，即可更新当前工作环境，让当前文件保持最新。更新完成后可以在使用log查看一下，会发现多出一个历史版本
...
- **--cached** 从暂存区域移除，但仍然保留在当前工作目录中

  > 如果想把文件从暂存区域移除，但仍然希望保留在当前工作目录中，换句话说，仅是从跟踪清单中删除，使用**--cached**选项即可

- **-r** 递归删除，即如果后面跟的是一个目录做为参数，则会递归删除整个目录中的所有子目录和文件；进入某个目录中，执行此语句，会删除该目录下的所有文件和子目录。

```
git rm -f runoob.txt 
```

#### git rm后恢复文件：git reset、git checkout
...
- git checkout <name>
  ...
  git checkout的作用是检出，如果是文件的话，会放弃对文件的缓存区操作，但是要使用reset重置一下变更才行，如果是分支的话会切换分支。

  

  ![img](git.assets/20210101124051184.png)

使用git checkout -b参数来创建一个分支，创建完成分支后会自动切换过去，等价于`git branch dev+git checkout dev`
...
#### github将本地仓库关联到远程仓库：git remote add 
...
这里输入yes就可以了，我们不需要登录，因为github上的ssh列表里有这台机器
